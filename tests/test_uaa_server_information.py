...
TOKEN = "dummy-token"
ERROR_RESPONSE = {"error": "UAA server is not running."}
...
@patch('uaa_server_information.requests.get', side_effect=requests.exceptions.ConnectionError)
def test_get_server_information_connection_error(mock_get):
    client = UAAServerInformationClient(BASE_URL, TOKEN)
    result = client.get_server_information()
...
@patch('uaa_server_information.requests.get', side_effect=requests.exceptions.ConnectionError)
def test_get_openid_configuration_connection_error(mock_get):
    client = UAAServerInformationClient(BASE_URL, TOKEN)
    result = client.get_openid_configuration()
    assert result == ERROR_RESPONSE
    mock_get.assert_called_once()
...
@patch('uaa_server_information.requests.post', side_effect=requests.exceptions.ConnectionError)
def test_create_passcode_connection_error(mock_post):
    client = UAAServerInformationClient(BASE_URL, TOKEN)
    result = client.create_passcode()
    assert result == ERROR_RESPONSE
    mock_post.assert_called_once()
...
@patch('uaa_server_information.requests.get', side_effect=requests.exceptions.ConnectionError)
def test_get_passcode_connection_error(mock_get):
    client = UAAServerInformationClient(BASE_URL, TOKEN)
    result = client.get_passcode()
    assert result == ERROR_RESPONSE
    mock_get.assert_called_once()
...
@patch('uaa_server_information.requests.get', side_effect=requests.exceptions.ConnectionError)
def test_get_auto_login_connection_error(mock_get):
    client = UAAServerInformationClient(BASE_URL, TOKEN)
    result = client.get_auto_login()
    assert result == ERROR_RESPONSE
    mock_get.assert_called_once()
...
@patch('uaa_server_information.requests.post', side_effect=requests.exceptions.ConnectionError)
def test_create_auto_login_connection_error(mock_post):
    client = UAAServerInformationClient(BASE_URL, TOKEN)
    result = client.create_auto_login("user", "pass")
    assert result == ERROR_RESPONSE
    mock_post.assert_called_once()
...
@patch('uaa_server_information.requests.get', side_effect=requests.exceptions.ConnectionError)
def test_perform_login_connection_error(mock_get):
    client = UAAServerInformationClient(BASE_URL, TOKEN)
    result = client.perform_login("code123", "clientid123")
    assert result == ERROR_RESPONSE
    mock_get.assert_called_once()
